...
(function ($) {
    ...
    TimeSlider.DEFAULTS = {
        ...
        time_zone: null,                       // optional IANA timezone string for labels
    ...
    TimeSlider.prototype.set_options = function (options) {
        ...
        if (options.time_zone !== undefined && options.time_zone !== this.options.time_zone) {
            this.options.time_zone = options.time_zone;
            this.remove_graduations();
            this.add_graduations();
            this.set_time_caret_position();
            this.set_contest_caret_position();
    ...
    TimeSlider.prototype.get_date_parts = function(timestamp) {
        var datetime = new Date(timestamp);
        if (!this.options.time_zone) {
            return {
        ...
        try {
            var parts = new Intl.DateTimeFormat('en-US', {
                timeZone: this.options.time_zone,
                year: 'numeric',
                month: '2-digit',
                day: '2-digit',
                hour: '2-digit',
    ...
    TimeSlider.prototype.timestamp_to_date = function(timestamp) {
        var parts = this.get_date_parts(timestamp);
        var hour = parts.hour % 12;
        if (hour === 0) hour = 12;
        var ampm = parts.hour >= 12 ? "PM" : "AM";
        return hour + ':' +
         ('0' + parts.minute.toString()).substr(-2) + ' ' + ampm;
    };
    ...
    TimeSlider.prototype.graduation_title = function(datetime) {
        var parts = this.get_date_parts(datetime.getTime());
        if (parts.hour == 0 && parts.minute == 0 && parts.second == 0) {
            return ('0' + parts.day.toString()).substr(-2) + '.' +
                ('0' + parts.month.toString()).substr(-2) + '.' +
                parts.year;
        }
        var hour = parts.hour % 12;
        if (hour === 0) hour = 12;
        var ampm = parts.hour >= 12 ? "PM" : "AM";
        return hour + ':' + ('0' + parts.minute.toString()).substr(-2) + ' ' + ampm;
    };
    ...
    TimeSlider.prototype.add_graduations = function() {
        ...
        for (var i = -4; i <= num_steps; i++) {
            ...
            var dateParts = this.get_date_parts(minute_caret);
            if (dateParts.hour == 0 && dateParts.minute == 0) {
                caret_class = 'big';
            }
...
//              console.log(this.options.contest_timestamp);
    ...
    TimeSlider.prototype.set_ruler_position = function(diff_x) {
        ...
        this.$ruler.children('.graduation').each(function () {
            ...
            var dateParts = _this.get_date_parts(minute_caret);
            if (dateParts.hour == 0 && dateParts.minute == 0) {
                caret_class = 'big';
            } else if (minute_caret / (60 * 1000) % medium_step == 0) {
                caret_class = 'middle';
